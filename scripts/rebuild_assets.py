...
from src.ingestion.pipeline import OUTPUT_DIR as PROCESSED_DIR
...
# 0. Prune stale duplicate artifacts (Issue #13 V2.6)
print("\n[0] Pruning stale duplicate artifacts...")
prune_result = prune_all(PROCESSED_DIR, HIERARCHY_DIR)
print(
    f"    Removed processed: {prune_result.removed_processed}, "
    f"hierarchy: {prune_result.removed_hierarchy}"
)

# 0b. Provenance retention cleanup (Issue #13 V2.7)
print("\n[0b] Cleaning provenance retention...")
provenance_report = ProvenanceStore(settings.QA_PROVENANCE_DIR).cleanup()
print(
    f"    Provenance removed: {provenance_report.removed}, remaining: {provenance_report.remaining}"
)
...
print(
    f"    Section 10: {'FOUND as standalone' if sec10 else 'MISSING'} "
    f"({sec10[0]['node_id'] if sec10 else 'N/A'})"
)
print(
    f"    Section 72: {'FOUND as standalone' if sec72 else 'MISSING'} "
    f"({sec72[0]['node_id'] if sec72 else 'N/A'})"
)
